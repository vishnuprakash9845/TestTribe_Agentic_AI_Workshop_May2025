...
Run via module mode for reliable imports:
  python -m src.agents.assignments.day4b_jira_slack_gap --input data/requirements/login.txt
...
def main() -> None:
    ...
    logger.info("ℹ️  Starting Jira/Slack gap analysis step")
    try:
        # Load test cases from the JSON file
        data = json.loads(Path(LAST_RAW_JSON.relative_to(
            ROOT)).read_text(encoding="utf-8"))

        # Prepare Slack message
        lines = [":memo: *LLM Test Case Update*",
                 f"*Total test cases created:* {len(data)}"]

        # Add test case titles
        for tc in data:
        ...
        logger.info("Posted Slack update for %d test cases.", len(data))
...
# 2. Run the script:
#  ```
#   python -m src.agents.assignments.day4b_jira_slack_gap --input path/to/requirement.txt
#   ```
#   python -m src.agents.assignments.day4b_jira_slack_gap --input data/requirements/login.txt
