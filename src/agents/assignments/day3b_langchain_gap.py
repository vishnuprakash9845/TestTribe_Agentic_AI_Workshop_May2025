...
SYSTEM_PROMPT = (
    PROMPTS_DIR / "gap_testcase_system.txt").read_text(encoding="utf-8")
USER_TEMPLATE_STR = (
    PROMPTS_DIR / "gap_testcase_user.txt").read_text(encoding="utf-8")
...
def main() -> None:
    ...
    logger.info(
        f"✅ Wrote {len(rows)} requirements to: {OUT_CSV.relative_to(ROOT)}")
    logger.info(
        f"ℹ️  Raw model output saved at: {LAST_RAW_JSON.relative_to(ROOT)}")
...
#   python -m src.agents.assignments.day3b_langchain_gap --input data/requirements/shopping_cart.txt
